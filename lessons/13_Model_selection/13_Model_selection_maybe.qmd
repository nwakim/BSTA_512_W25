...
3.  Know the formula for and function for various model fit statistics
...
|-------------------------------|-----------------------------------------|
| R-squared / Adjusted R-squared       | $Adj. R^2 = 1 - \frac{SSE/(n-p-1)}{SSY/(n-1)}$                                        |
| Mallow's $C_p$                       | $C_p = \Bigg[ \dfrac{\widehat\sigma^2_p}{\widehat\sigma^2_{max}} - 1 \Bigg](n-p) + p$ |
| Akaike information criterion (AIC)   | $AIC = n\log(SSE) - n \log(n) + 2(p+1)$                                               |
| Bayesian information criterion (BIC) | $BIC = n\log(SSE) - n\log(n) + log(n)\cdot(p+1)$                                      |
